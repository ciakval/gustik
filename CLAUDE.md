...
- **`backend/`** — Node.js 24 + Fastify 5.11.x + better-sqlite3 13.0.x + `@fastify/websocket` + `@fastify/static`. `npm install && npm test` (43 tests, built-in `node:test`, zero extra assertion deps — see cerebrum.md). `npm start` runs the server (needs `INGEST_TOKEN` env var). Serves the dashboard (`src/static/`: live view, historical Chart.js graph, operation manual) as static files, no frontend build step. `Dockerfile`/`compose.yaml` are build-verified by CI on every push (no `docker` binary in this devcontainer, so never locally) and, on `main`, the image is pushed to GHCR and deployed for real to `gustik.remesh.cz` — see `TODO.md`.
