...
1. ~~Docker build verification~~ — **done 2026-08-10**: backend is built and running for real on `bombur.remesh.cz`, live at https://gustik.remesh.cz. **Remaining**: flip `DEPLOY_ENABLED` repo variable to `true` once ready for pushes to `main` to auto-deploy (currently a verified no-op).
